...
| V018_INVALID_SLUG_FORMAT | Invalid Slug Format | error | `validateI4RequiredFields`, `validateSlugFormats` | `feature_slug` or `program_slug` or IMPL slug is not kebab-case (`^[a-z0-9]+(-[a-z0-9]+)*$`) |
...
| V024_PROGRAM_INVALID | Program Invalid | fatal | `FullValidateProgram` | PROGRAM manifest fails to parse |
...
| V039_INVALID_FIELD_VALUE | Invalid Field Value | error | `validateI4RequiredFields` | `verdict` is not `SUITABLE`, `NOT_SUITABLE`, or `SUITABLE_WITH_CAVEATS` |
...
| V046_PARSE_ERROR | Parse Error | error | `ValidateDuplicateKeys`, `FullValidateProgram` | Raw YAML cannot be parsed at all |
...
**I4 — Required Top-Level Fields (V005, V018, V039):** `title`, `feature_slug`, and `verdict` must be non-empty. `feature_slug` must be kebab-case. `verdict` must be one of `SUITABLE`, `NOT_SUITABLE`, `SUITABLE_WITH_CAVEATS`.
...
**V048 (LOC Budget):** When `repoPath` is provided, the lines of all `action=modify` files per agent are summed. Agents exceeding 2000 total lines receive a blocking error listing the largest files by LOC.
