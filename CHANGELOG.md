...
### Fixed (2026-04-08)
- **`validate-briefs` adds `wave_reference_invalid` check** — `ValidateBriefs` in `pkg/protocol/brief_validator.go` now parses "Wave N Agent X" patterns in agent brief prose and cross-checks the wave number against the actual wave assignment in the IMPL doc. Errors like "brief says 'Wave 2 Agent G' but Agent G is in Wave 3" are caught at write time (via `validate_impl_on_write` hook) and at scout exit (via `validate_agent_completion` SubagentStop hook), before the critic gate runs. Case-insensitive match handles "Wave 2 agent G" and multi-generation IDs (A2, B3, etc.). New helpers: `buildAgentWaveMap`, `extractWaveAgentRefs`, `waveAgentRef` type.
- **Case-insensitive repo resolution** — `ResolveTargetRepos`, `ResolveAgentRepo`, `ValidateFileExistenceMultiRepo`, and `CreateWorktrees` now resolve repo names case-insensitively. When `repo: lsp-mcp-go` is used in an IMPL doc but the actual directory is `LSP-MCP-GO`, the engine reads the parent directory to find the canonical case-preserving name instead of failing. New helpers: `resolveSiblingCaseInsensitive` (reads parent dir, matches case-insensitively), `configLookupCI` (case-insensitive config map lookup). Eliminates the need for manual `repos` mappings in `saw.config.json` for case mismatches.
- **Merge verification uses branch tip, not completion report SHA** — `mergeAgentsSingleRepo` and `mergeAgentsMultiRepo` now verify the merged branch's tip commit is reachable from HEAD via `git merge-base --is-ancestor`, instead of checking `report.Commit` from the agent's completion report. If the branch tip IS reachable but `report.Commit` is NOT (e.g., the agent amended/rebased after recording the completion SHA), the merge succeeds with a warning log. Previously this was a fatal `MERGE_VERIFICATION_FAILED` error requiring `--skip-merge` workaround. The branch tip is the authoritative reference for what was actually merged.
...
|---------|------|----------|
| [1.11.0] | 2026-04-01 | Package review progress — 11 packages reviewed (agent, errparse, notify, suitability, worktree, queue, scaffold, collision, hooks, idgen, deps); 6 packages closed with IMPLs executed (agent, errparse, notify, suitability, worktree, queue); 5 packages with IMPLs scouted and ready (collision: 22 findings/3 agents/42%→80%+ coverage, hooks: 6 findings/4 agents, idgen: 3 findings/2 agents/98.3% maintained, deps: 20 findings/8 agents/85.9%→92%+ coverage, scaffold: 4 low findings); total findings identified: 55+ across collision/hooks/idgen/deps packages |
| [1.10.0] | 2026-04-01 | Queue package review fixes — 2-wave IMPL (2 agents): added Q001-Q006 error codes; `Result[T]` migration for `completedSlugs()`; replaced 9 hardcoded error strings; removed unreachable nil check; added 4 error path tests (`TestAdd_MkdirFails`, `TestList_CorruptedYAML`, `TestCompletedSlugs_CompleteDir_ReadFailure`, `TestUpdateStatus_SaveYAMLFails`); test coverage: 19 tests passing (15 existing + 4 new) |
| [1.9.0] | 2026-04-01 | Worktree package review fixes — 2-wave IMPL (4 agents): `Result[T]` migration for `worktree.Create()` → `result.Result[CreateData]`; removed unused `worktrees` field from `agent.Runner`; fixed orchestrator callers + test assertions; fixed post-merge: 18 direct calls + 6 engine calls to `agent.NewRunner` |
| [1.8.0] | 2026-04-01 | Code review return-type migration + validation hardening — `ReviewDiff`/`RunCodeReview` return `result.Result[ReviewResult]` (replaces `(*ReviewResult, error)`); `ReviewOpts.BaseURL` field replaces global `testBaseURL`; `validateReviewResponse` helper enforces non-empty dimensions, scores 0–100, non-empty summary; empty diff returns `Overall: 100, Skipped: true` (was `Overall: 0`); truncation message includes actual byte count; git fallback preserves `firstErr` and reports both errors; API error message includes model and diffBytes; `RunE` uses `res.IsFatal()`/`res.GetData()` so skipped reviews no longer fail the command; 2 new tests: `TestValidateReviewResponseFailure`, `TestReviewDiffEmptyDiffScore` |
| [1.7.0] | 2026-04-01 | Language-agnostic workspace manager — new `pkg/engine/workspace/` package with `WorkspaceManager` interface (`Language`, `Detect`, `Setup`, `Restore`) and `DetectAndSetup`/`DetectAndRestore` dispatchers (non-fatal, fires per-language `onEvent` callbacks); `GoWorkspaceManager` fixes "module appears multiple times in workspace" by reading root `go.mod` and skipping `go work init` for single-module repos; `TypeScriptWorkspaceManager` adds project references to `tsconfig.json` + per-worktree `tsconfig.worktree.json`; `RustWorkspaceManager` surgically appends worktree paths to `[workspace] members` in `Cargo.toml`; `PythonWorkspaceManager` updates `pyrightconfig.json` extraPaths and all three `pyproject.toml` `[tool.pyright] extraPaths` cases; engine wiring replaces `StepGoWorkSetup`/`StepGoWorkRestore` with `workspace.DetectAndSetup`/`DetectAndRestore`; `NoWorkspaceSetup bool` replaces deprecated `NoGoWork` on step types; `--no-workspace-setup` flag added to `prepare-wave` (`--no-gowork` kept as deprecated alias) |
| [1.6.0] | 2026-03-31 | Critic gate fixes + program status sync — `E37Required()` extracted as single authoritative threshold function in `critic_gate.go`; `prepare-tier` now respects E37 threshold (sub-threshold IMPLs no longer blocked); `checkCriticReview` refactored to use `E37Required` (eliminates inline duplication); `RunTierGate` now reads IMPL completion state from disk via `enrichIMPLStatusesFromDisk` (eliminates manual `update-program-impl` loop after `finalize-wave`); 3 new tests for `PrepareTier` E37 scenarios; 2 new tests for `RunTierGate` disk/fallback; 7 new tests for `E37Required` boundary conditions |
| [1.5.0] | 2026-03-31 | Code review bug fixes (PROGRAM mode) — 3 parallel IMPLs via `/saw program --impl`: `config-hardening` (3 agents: 0600 file permissions on `Save()`, legacy migration error handling, `slog.Warn` on default config, `CodeConfigIOFailed` error code, nil guards in `GetWaveState`/`GetAllWaveStates`, dead `GateResults` field removed, wave context in errors, `FindConfigPath` dedup), `collision-fixes` (1 agent: `context.TODO()`→`ctx`, `"."` path normalization, `slog.Debug` on git skip, concurrency comment, root-package test), `commands-fixes` (2 agents: tab-only Makefile indent, recursive `resolveTargetChains`, prettier→Fix classification, broken `contains()` helper, toolchain tiebreak comment, `strings.Title`→`cases.Title(language.Und)` with `golang.org/x/text`); daemon.go `completedCount` indentation fix (blocked IMPLs no longer counted as completed); all 41 packages green |
| [1.4.0] | 2026-03-31 | Reference documentation — 14 new docs in `docs/reference/` covering manifest schema, program schema, all 32 protocol rules (E10–E44), 50+ validation invariants, wiring declarations, conflict prediction, quality gates, result types, autonomy levels, critic workflow, completion reports, reactions config, and observability; `cli-reference.md` updated with 7 previously undocumented commands (`auto`, `detect-wiring`, `predict-conflicts`, `pre-wave-validate`, `run-integration-agent`, `run-integration-wave`, `resolve-impl`); `docs/reference/README.md` index added; `CLAUDE.md` added with docs contract and design principles; stale wave planning docs removed (`E35_DETECTION.md`, `WAVE_EXECUTION_WITH_E35.md`, `IMPLEMENTATION_SUMMARY.md`); `error-codes.md` moved to `docs/reference/` |
| [1.3.9] | 2026-03-31 | Miscellaneous cleanup — typed all 11 `Event.Data interface{}` event bus payloads in `pkg/engine/runner_data_types.go`; replaced all 14 `map[string]interface{}` publish call sites in `runner.go` with typed structs; added typed `GetValue[T]`/`SetValue` accessors for `pipeline.State.Values`; added private typed structs for Slack and Discord notify payloads; synced `docs/error-codes.md` with N018–N084 range (was missing all engine operation codes); deleted dead `pkg/orchestrator/setters.go` and moved `SetLogger` into `orchestrator.go` |
| [1.3.8] | 2026-03-31 | Protocol + backend cleanup — extracted shared `ExecuteTool` function to `pkg/agent/backend/execute_tool.go`, removed duplicate implementations from api, bedrock, and openai backends; added `CheckConstraints` post-hoc enforcement in CLI backend's `RunStreamingWithTools`; fixed `ScaffoldFile` struct YAML tag (`yaml:"file"` → `yaml:"file_path"`, was causing silent data loss on YAML roundtrip); changed `pipeline.State.Errors` from `[]error` to `[]result.SAWError` |
| [1.3.7] | 2026-03-31 | Context propagation — removed `ctx` from `LoadYAML`/`SaveYAML` (no I/O, ctx was a no-op); added `ctx` to all functions that run git/shell commands: `CreateWorktrees`, `MergeAgents`, `VerifyBuild`, and ~20 more protocol functions; updated all call sites in engine and CLI layers |
| [1.3.6] | 2026-03-31 | Engine structural debt — extracted `runOneWave` helper to deduplicate `StartWave`/`startWaveWithGate` bodies; extracted `firstRepoOpts` helper (eliminated 7 copy-paste patterns in `finalize.go`); added `resolveAgentBranch` helper in `branchname.go` for slug-scoped branch resolution with legacy fallback |
| [1.3.5] | 2026-03-31 | Engine error code catalog — replaced 67 ad-hoc `ENGINE_*` string constants with typed `result.SAWError` codes N018–N084; all engine operation events now use structured error codes |
| [1.3.4] | 2026-03-31 | Crash fixes + error string — fixed panic when `ImplState` is nil in `set_impl_state_cmd.go`; fixed incorrect assertion in `close-impl` that blocked completion; fixed error string formatting (no capitalization, no trailing punctuation) |
| [1.3.3] | 2026-03-30 | Observability improvements — `ObsEmitter` interface introduced in `pkg/engine/engine.go` (exposes both `Emit` + `EmitSync`); all 4 engine opts structs (`RunScoutOpts`, `FinalizeWaveOpts`, `MarkIMPLCompleteOpts`, `RunWaveTransactionOpts`, `TierLoopOpts`) accept `ObsEmitter` instead of concrete `*observability.Emitter` (interface satisfied structurally, no caller changes); 3 highest-signal call sites migrated to `EmitSync` with `slog.Warn` (`impl_complete`, `scout_launch`, `wave_merge`); `result.ToErrors([]SAWError) []error` added to `pkg/result` as exported helper; `sawErrsToErrors` in `cmd/sawtools/cleanup.go` deleted, all 10 call sites across 8 files migrated to `result.ToErrors`; LOC budget error message now includes per-file line count breakdown (sorted descending) for actionable split guidance |
| [1.3.2] | 2026-03-30 | Post-migration cleanup — removed `MergeAgentsData.Success` legacy compatibility shim (replaced by `result.Result[T].IsSuccess()` across all 4 callers); wired `CheckAgentLOCBudget` into `FullValidate` (LOC budget now enforced end-to-end via `sawtools validate --repo-dir`); `emitWaveMerge`/`emitWaveStart` in CLI obs helpers migrated from fire-and-forget `Emit` to blocking `EmitSync` with `slog.Warn` (void signatures preserved, no caller changes); 9 `fmt.Errorf("%v", res.Errors)` calls in `cmd/sawtools/` replaced with `errors.Join` + `%w` (preserves error chain for `errors.Is`/`errors.As`); `sawErrsToErrors` helper added to bridge `[]result.SAWError` → `[]error` |
| [1.3.1] | 2026-03-30 | SAW system hardening (5 validation gaps closed) — ghost file detection: `sawtools validate` now accepts `--repo-dir` and activates `ValidateFileExistence` to catch IMPL docs referencing non-existent files (`V001_FILE_MISSING`); LOC budget enforcement: `CheckAgentLOCBudget` blocks agents owning >2,000 lines total (`V048_AGENT_LOC_BUDGET`) preventing context exhaustion; I1 dead-code fix: `prepare.go` I1 guard was filtering for string `"I1_VIOLATION"` (never emitted) instead of `result.CodeDisjointOwnership` — one-line fix activates the existing guard; completion report enforcement: `UpdateStatus` now requires `Commit` when setting `status=complete`, blocking sparse reports that pass `update-status` but fail `verify-commits` downstream; build gate hook: `DiscoverBuildGate` added, `RunPreCommitCheck` now runs lint then `go build ./...` (fast-fail order) catching partial migrations before commit |
| [1.3.0] | 2026-03-30 | result.Result[T] migration complete — all error-only returns migrated to `result.Result[T]` (NewSuccess/NewPartial/NewFailure) across `pkg/protocol` (Save, Load, gates, freeze, stubs, merge_log, conflict_predict), `pkg/pipeline` (Run, executeStep), `pkg/observability` (EmitSync), `pkg/autonomy` (LoadConfig/SaveConfig), `pkg/orchestrator` (16 functions), and `pkg/engine` (StartWave, RunSingleWave, MergeWave, RunVerification, 19 runner functions); `runner_data_types.go` extracted for StartWaveData/WaveData/MergeData/VerificationData; 3-wave parallel IMPL (11 agents: I-R2) across all packages; 42 packages passing |
| [1.2.4] | 2026-03-29 | Pre-commit gate repo scoping fix — `DiscoverLintGate()` now checks `gate.Repo` against current repo before returning a lint command; cross-repo IMPLs with repo-scoped lint gates (e.g., `go vet ./...` scoped to `scout-and-wave-go`) no longer run in unrelated repos (e.g., protocol repo with no Go module); falls back to next matching gate or `saw.config.json` when all lint gates target other repos; 3 new tests (cross-repo skip, same-repo match, all-other-repo fallback) |
| [1.2.3] | 2026-03-28 | Pre-commit cross-repo validation fix — `IMPLManifest.TargetsRepo(repoDir)` method added with 3-tier logic: explicit `Repository`/`Repositories` field matching (absolute paths), `FileOwnership.Repo` field matching (repo names), file existence fallback for legacy IMPLs; `DiscoverLintGate()` now skips IMPLs that don't target current repo; resolves issue where IMPLs stored in repo A targeting files in repo B would run lint gates in wrong repo (e.g., `cargo clippy` in non-Rust repos), eliminating need for `--no-verify` commits; integration agent CLI workflow merged: `cmd/sawtools/run_integration_agent_cmd.go` (E26 reactive integration), `cmd/sawtools/run_integration_wave_cmd.go` (E27 planned integration waves), enhanced `finalize_wave.go` stderr output with gap summary and next-step guidance, command registration in `main.go`; baseline test fix (removed stale `WrittenAt` assertion from `completion_report_test.go`); 12 new tests (5 run-integration-agent, 7 run-integration-wave) |
| [1.2.2] | 2026-03-27 | Baseline gate diagnostic output — `recordStepWithData()` passes `BaselineData` through `StepResult.Data`; `prepare-wave` calls `FormatBaselineOutput` on baseline failures, showing stdout/stderr from failed gates instead of opaque "baseline_verification_failed" message; fixes UX gap where users had no diagnostic information on pre-flight gate failures |
| [1.2.1] | 2026-03-27 | Schema drift fix — removed `WrittenAt` field from `CompletionReport` struct (was causing H5 gate failures); `sawtools set-completion` no longer writes `written_at` timestamps that violate the completion report schema; fixes Wave 2 launch blocker where agents completed successfully but orchestrator validation failed on next wave |
| [1.2.0] | 2026-03-27 | Shared data structure detection — `pkg/analyzer/shared_types.go` detects types referenced by 2+ agents (Go, Rust, TypeScript, Python import pattern matching, fuzzy path resolution, circular dependency detection); `sawtools detect-shared-types` CLI command outputs YAML/JSON scaffold candidates; 12 detection tests + 4 CLI tests; prevents I1 violations from duplicate type definitions across agents |
| [1.1.0] | 2026-03-26 | Audit findings unification — `RetryResult` deleted, fields merged into `RetryAttempt`, `RetryLoop.Run()` now returns `*RetryAttempt` (Finding 3); `CompletionStatus` typed string enum replaces bare `string` on `CompletionReport.Status` with constants `StatusComplete/StatusPartial/StatusBlocked` propagated across all packages (Finding 6); YAML call-site exemption catalog added to `yaml_io.go` documenting all 16 legitimate direct call sites (Finding 5); godoc and call-order hierarchy added to `FullValidate`/`Validate`/`ValidateIMPLDoc`/`ValidateBytes` in `doc.go` (Finding 7); `%v`→`%w` error-wrap fixes in `cmd/sawtools/` for true Go error values (Finding 10) |
| [1.0.0] | 2026-03-25 | Error code unification — deleted `pkg/protocol/error_codes.go` (E-series, zero callers) and `pkg/protocol/schema_errors.go` (SV01 constants); all 22+ files in `pkg/protocol/` and `pkg/errparse/` now use V/W/B/G/A/N/P/T series constants from `pkg/result/codes.go`; 12 new constants added (V036–V046 + W002); `pre-wave-gate` bug fixed (W001 advisory warnings no longer block wave execution) |
| [0.99.0] | 2026-03-25 | Protocol architectural unification — `ExtractIMPLSlug` (canonical slug extraction with `FeatureSlug` → filename-strip → path fallback priority); `IMPLStateToStatus` (fixes `StateReviewed` → `"pending"` behavioral bug); `ComputeTierAssignments` + `BuildFileOwnershipMap` canonical tier/ownership helpers; `ResolveIMPLPathOrAbs`/`ResolveIMPLPath` unified path resolvers; 5 new typed error codes (N015-N017, B008, P007) replacing scattered inline strings; `cmd/saw/import_impls_cmd.go` migrated to all canonical functions (-57 lines); new tests for `impl_slug`, `state_status`, `program_conflict`, `state_status` |
| [0.98.0] | 2026-03-25 | Webhook adapter unification — unified field names (`mode`, `token`, `destination`) across Slack/Discord/Telegram; `readWithFallback` for backward compat with old field names (`bot_token`, `channel`, `chat_id`); backward compat tests |
| [0.97.0] | 2026-03-25 | Webhook notification system — `pkg/notify/` extractable library with Dispatcher, Registry, Slack (webhook + bot token), Discord, Telegram adapters; Block Kit/embed/Markdown formatters; `init()` auto-registration; 25+ tests; zero SAW-specific imports |
| [0.96.0] | 2026-03-25 | `sawtools init` + pre-wave-gate fix + MR02 validation — zero-config project initialization with auto-detection (Go, Rust, Node, Python, Ruby, Makefile); pre-wave-gate E37 threshold fixed (wave 1 agents + file_ownership repos, not total agents); `MR02_UNSCOPED_GATE` validation blocks cross-repo IMPLs with un-scoped quality gates |
...
| [0.89.0] | 2026-03-23 | I4 completion report gate + I1 layer 2 git ownership — `finalize-wave` blocks when completion reports missing (I4); `BashExecutor` post-execution check warns when git commands modify unowned files (reads `.saw-ownership.json`); E21B cross-repo baseline gates in `prepare-wave` (runs gates on ALL target repos before worktree creation) |
| [0.88.0] | 2026-03-23 | Provider credentials — `BedrockProfile`/`BedrockRegion`/`BedrockAccessKeyID`/`BedrockSecretAccessKey`/`BedrockSessionToken`/`AnthropicKey` fields on `backend.Config`; Bedrock client SSO profile support via `config.WithSharedConfigProfile`; orchestrator wiring for all credential fields; `RepoEntry` gains `BuildCommand`/`TestCommand` for per-repo gate config |
...
| [0.83.0] | 2026-03-22 | Program-tier isolation engine — `MergeTarget` field on RunWaveFullOpts/FinalizeWaveOpts; `MergeAgents` 4th arg (mergeTarget); `--merge-target` CLI flags on finalize-wave/prepare-wave; RunTierLoop creates IMPL branches via ProgramBranchName |
| [0.82.0] | 2026-03-22 | close-impl batching command — combines mark-complete + archive + update-context + clean worktrees + clean .saw-state; empty slug directory cleanup after worktree removal; docs rewrite (architecture, orchestration, api-endpoints, sse-events, binaries, determinism-roadmap) |
| [0.81.0] | 2026-03-21 | Determinism enforcement — real I1/I2/I6 constraint middleware in `pkg/tools/constraint_enforcer.go` replaces passthrough stubs (unconditional `init()` override); `ValidateCompletionReportClaims` (uses `git cat-file -e` for correct SHA existence check) + `ValidateFileOwnershipCoverage` in `pkg/protocol/`; `VerifyDependenciesAvailable` + `ValidateGateInputs` (git diff vs reported files) in `pkg/protocol/`; `PreMergeValidation` added to `merge_agents.go` wired into both single/multi-repo merge paths; `prepare-wave` Step 0b1.5 (H2 fatal dep verification) + Step 0b1.6 (H1 ownership coverage warning) |
| [0.80.0] | 2026-03-21 | Baseline gate enforcement — `RunBaselineGates` in `pkg/protocol/baseline_gates.go` wraps `RunPreMergeGates` with gate-cache integration and structured `BaselineResult` (per-gate results, `FromCache`, `CommitSHA`); `FormatBaselineOutput` per-gate PASS/FAIL/SKIP formatter with `ParsedErrors` priority and cache annotation in `cmd/saw/baseline_output.go`; `prepare-wave` replaces 27-line inline baseline block with `RunBaselineGates` call + `--no-cache` flag; structured JSON to stdout on baseline failure for programmatic consumers |
| [0.79.0] | 2026-03-21 | sawtools CLI gaps — `validate-integration --wave N` CLI command wrapping `ValidateIntegrationGaps`; `set-impl-state` command for direct state transitions; `WorktreesAbsent(wave *Wave) bool` helper (replaces `IsSoloWave`); `IntegrationGapSeverityThreshold` field on `IMPLManifest`; `finalize-wave` wires integration gap detection post-merge |
| [0.78.0] | 2026-03-21 | Scaffold validation + freeze check in `prepare-wave` pre-flight — step 0d blocks worktree creation when any scaffold file is not yet committed (`AllScaffoldsCommitted`); step 0e blocks on interface freeze violations when worktrees already exist (`CheckFreeze`); both return structured JSON errors |
| [0.77.0] | 2026-03-21 | Repo mismatch hardening — `ResolveTargetRepos` + `ValidateRepoMatch` in `pkg/protocol/repo_resolve.go`; `prepare-wave` step 0b1 validates agent repos match configured repos before worktree creation; step 0b1.5 checks file existence across multiple repos; prevents agents running in wrong repository |
| [0.76.0] | 2026-03-21 | Auto-program-from-IMPLs — `CheckIMPLConflicts` (greedy graph coloring for disjoint tier assignment) in `pkg/protocol/program_conflict.go`; `GenerateProgramFromIMPLs` (loads IMPLs, builds `PROGRAMManifest` with auto-assigned tiers) in `pkg/protocol/program_generator.go`; `sawtools create-program --from-impls` + `sawtools check-impl-conflicts` CLI commands |
...
| [0.71.0] | 2026-03-19 | AI code review gate SDK (ai-code-review-gate) — `pkg/codereview/` package: `ReviewDiff`, `CodeReviewResult`, `RunCodeReview` with configurable model + system prompt, 8 tests; `sawtools run-review` CLI command |
| [0.70.0] | 2026-03-19 | DAG prioritization (planner-dag-prioritization) — `PriorityScore`/`PriorityReasoning` on `ProgramIMPL`, `ConcurrencyCap` on `ProgramTier` in `pkg/protocol/program_types.go`; BFS `UnblockingScore` + stable-sort `PrioritizeIMPLs` in `pkg/protocol/program_prioritizer.go` (9 tests); `ScoreTierIMPLs` + `ScoredIMPLOrder` in `TierAdvanceResult`, wired into `AdvanceTierAutomatically` |
