  ...
  polywave-tools close-impl docs/IMPL/IMPL-feature.yaml --date 2026-03-22`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if entries, err := os.ReadDir(sawStatePath); err == nil {
				...
				if activeIMPLs == 0 {
					...
					if stateCleanedCount > 0 {
						fmt.Fprintf(os.Stderr, "close-impl: cleaned %d .polywave-state dir(s) (no active IMPLs)\n", stateCleanedCount)
					}
				}
			}
			...
			if rmErr := git.Rm(projectRoot, manifestPath); rmErr != nil {
				...
				// Commit.
				commitMsg := fmt.Sprintf("chore: close-impl %s [polywave:complete]", manifest.FeatureSlug)
				var commitErr error
