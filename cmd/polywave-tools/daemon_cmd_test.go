...
// TestDaemonCmd_DefaultConfig verifies the daemon loads config from polywave.config.json
...
func TestDaemonCmd_DefaultConfig(t *testing.T) {
	...
	// (since DefaultConfig is used when polywave.config.json is missing).
	if err != nil && strings.Contains(err.Error(), "load config") {
		t.Errorf("expected default config to be used when polywave.config.json is missing, got error: %v", err)
	}
}
