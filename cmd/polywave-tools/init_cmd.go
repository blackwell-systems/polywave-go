...
After running init, use /saw scout "feature" in Claude Code or saw serve for the web UI.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if result.InstallResult.Verdict == "FAIL" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  go install github.com/blackwell-systems/polywave-go/cmd/sawtools@latest\n\n")
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  cd polywave-go && go build -o ~/.local/bin/polywave-tools ./cmd/sawtools\n")
			}
