...
func newInstallHooksCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install-hooks",
		Short: "Install SAW pre-commit quality gate hook into .git/hooks/pre-commit",
...
func runInstallHooks(repoDir string) error {
	...
	const snippet = `# SAW pre-commit quality gate (M4)
sawtools pre-commit-check --repo-dir "$(git rev-parse --show-toplevel)"`
	...
	if strings.Contains(existingContent, "sawtools pre-commit-check") {
