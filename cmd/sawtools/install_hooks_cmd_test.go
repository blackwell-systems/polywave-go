...
func TestInstallHooks_FreshInstall(t *testing.T) {
	...
	// Verify snippet
	if !strings.Contains(hookStr, "sawtools pre-commit-check") {
		t.Error("expected sawtools pre-commit-check in hook")
	}

	if !strings.Contains(hookStr, "# SAW pre-commit quality gate (M4)") {
