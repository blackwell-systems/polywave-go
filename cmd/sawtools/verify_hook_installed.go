...
The hook should block commits to main/master branches unless SAW_ALLOW_MAIN_COMMIT
...
func verifyHookInstalled(worktreePath string, waveNum int) (*HookVerificationResult, error) {
	...
	hasIsolationCheck := strings.Contains(hookContent, "SAW_ALLOW_MAIN_COMMIT") ||
		strings.Contains(hookContent, "SAW pre-commit guard")
