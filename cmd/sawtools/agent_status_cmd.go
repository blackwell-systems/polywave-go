...
func newAgentStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// try to find it via saw.config.json starting from the manifest's directory.
...
func buildCriticRow(m *protocol.IMPLManifest, wave protocol.Wave, repoDir string) AgentStatusRow {
	...
	if repoDir != "" && len(m.Waves) > 0 {
		...
		if baseCommit != "" {
			// Count commits matching [SAW:critic:<slug>] since the base commit.
			lines, err := git.LogOneline(repoDir, baseCommit+"..HEAD")
			if err == nil {
				count := 0
				searchStr := "[SAW:critic:" + m.FeatureSlug + "]"
