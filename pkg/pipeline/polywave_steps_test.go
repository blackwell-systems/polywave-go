...
// TestRequiredKeyErr_ReturnsPolywaveError verifies that requiredKeyErr returns
// a PolywaveError (not a plain fmt.Errorf string) so the Code field is preserved.
func TestRequiredKeyErr_ReturnsPolywaveError(t *testing.T) {
	...
	if !errors.As(err, &sawErr) {
		t.Fatalf("expected PolywaveError, got %T: %v", err, err)
	}
