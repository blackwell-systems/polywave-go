...
// ClassifyExport classifies an exported symbol into a category.
...
// IsIntegrationRequired determines if an exported symbol likely needs
// integration wiring based on naming patterns and category.
func IsIntegrationRequired(exportName string, category string) bool {
	// Functions with these prefixes are almost certainly meant to be called
	requiredPrefixes := []string{
		"New", "Build", "Register", "Run", "Start", "Init",
		"Create", "Setup", "Configure", "Handle", "Process",
		"Validate", "Execute", "Launch", "Open", "Connect",
	}
	...
	// Types are generally expected to be used
	if category == "type_usage" {
		return true
	}

	// Struct fields named with common integration patterns
	if category == "field_init" {
		return false // Fields are less likely to need explicit integration
	}
...
// the given export. Uses import graph and naming heuristics.
func SuggestCallers(repoPath string, exportPkg string, exportName string) ([]string, error) {
	var callers []string

	// Look for files that import the package containing the export
	pkgName := filepath.Base(exportPkg)
	if pkgName == "." || pkgName == "" {
		return callers, nil
	}
	...
	err := filepath.Walk(repoPath, func(path string, info os.FileInfo, err error) error {
		...
		// Skip files in the same package
		if filepath.Dir(rel) == exportPkg {
			return nil
		}

		// Check if file imports or references the package
		if fileContains(path, pkgName) {
